...
export function ResultsBody({
  ...
  switch (slug) {
    ...
    case 'outbound-campaign':
      return <CampaignsView result={result} />;
    default:
...
function CampaignsView({ result }: { result: Record<string, unknown> }) {
  const personas: Array<Record<string, unknown>> = Array.isArray(result.personas)
    ? (result.personas as Array<Record<string, unknown>>)
    : [];
  const title = stringOr(result.campaignTitle, 'Outbound Campaign');
  const summary = stringOr(result.executiveSummary, '');
  const plan = stringOr(result.agentPlan, '');
  const selfReview = stringOr(result.selfReviewNotes, '');
  const totalPersonas = numberOrUndef(result.totalPersonas) ?? personas.length;
  const highPriority = numberOrUndef(result.highPriorityCount);
  const totalSteps = numberOrUndef(result.totalSteps);

  const segments = Array.isArray(result.segmentsCovered)
    ? (result.segmentsCovered as unknown[]).filter(
        (s): s is string => typeof s === 'string',
      )
    : [];
  const topAngles = Array.isArray(result.topPerformingAngles)
    ? (result.topPerformingAngles as unknown[]).filter(
        (s): s is string => typeof s === 'string',
      )
    : [];
  const deprioritize = Array.isArray(result.segmentsToDeprioritize)
    ? (result.segmentsToDeprioritize as unknown[]).filter(
        (s): s is string => typeof s === 'string',
      )
    : [];
  const deliverability = Array.isArray(result.deliverabilityTips)
    ? (result.deliverabilityTips as unknown[]).filter(
        (s): s is string => typeof s === 'string',
      )
    : [];
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2 text-xs font-medium">
        ...
        {totalSteps !== undefined && (
          <div className="rounded bg-card px-3 py-2 text-foreground">
            <div className="text-lg font-bold">{totalSteps}</div>
            <div className="text-foreground/85">Total steps</div>
          </div>
        )}
        {highPriority !== undefined && (
          <div className="rounded bg-accent px-3 py-2 text-accent-foreground">
            <div className="text-lg font-bold">{highPriority}</div>
      ...
      {segments.length > 0 && (
        <div className="rounded-lg border border-border bg-card p-3">
          <p className="mb-2 text-sm font-semibold text-foreground">🎯 Segments covered</p>
      ...
      {topAngles.length > 0 && (
        <div className="rounded-lg border border-brand-100 bg-accent p-3">
          <p className="mb-2 text-sm font-semibold text-brand-700">
            ⭐ Top-performing angles across the batch
          </p>
          <ul className="space-y-1 text-sm text-foreground">
            {topAngles.map((s, i) => (
              <li key={i}>• {s}</li>
...
function PersonaRow({ persona }: { persona: Record<string, unknown> }) {
  const name = stringOr(persona.personaName, '—');
  const title = stringOr(persona.jobTitle, '');
  const company = stringOr(persona.company, '');
  const segment = stringOr(persona.segment, '');
  const hook = stringOr(persona.openingHook, '');
  const angle = stringOr(persona.matchedAngle, '');
  const proofPoint = stringOr(persona.matchedProofPoint, '');
  const reasoning = stringOr(persona.reasoning, '');
  const confidence = numberOrUndef(persona.confidence);
  const isHighPriority = Boolean(persona.isHighPriority);

  const steps = Array.isArray(persona.sequence)
    ? (persona.sequence as Array<Record<string, unknown>>).filter(
        (s): s is Record<string, unknown> => s && typeof s === 'object',
      )
    : [];
  const notes = Array.isArray(persona.personalizationNotes)
    ? (persona.personalizationNotes as unknown[]).filter(
        (s): s is string => typeof s === 'string',
      )
    : [];
  const flags = Array.isArray(persona.complianceFlags)
    ? (persona.complianceFlags as unknown[]).filter(
        (s): s is string => typeof s === 'string',
      )
    : [];
    ...
    <div className="rounded-lg border bg-card p-3 transition-shadow hover:shadow-brand-card">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 text-sm text-foreground/85">
            {[title, company, segment].filter(Boolean).join(' · ')}
          </p>
        </div>
        {confidence !== undefined && (
          <span
            className={`text-[11px] font-medium ${confidenceClasses}`}
            title="How well the persona signal matched a proof point from the brief. <70 = thin row data; rep should re-source."
          >
            {confidence}% confidence
          </span>
        )}
      ...
      {hook && (
        <div className="mt-2 rounded border border-border bg-muted/40 px-2.5 py-1.5 text-xs">
          <span className="font-medium text-foreground">🎣 Opening hook:</span>{' '}
          <span className="text-foreground/85">{hook}</span>
        </div>
      )}
      ...
      {(angle || proofPoint) && (
        <div className="mt-2 grid grid-cols-1 gap-1.5 text-xs sm:grid-cols-2">
          {angle && (
            <div className="rounded border border-brand-200 bg-accent px-2.5 py-1.5">
              <p className="text-[10px] font-medium uppercase tracking-wider text-brand-700">
                Matched angle
              </p>
              <p className="text-foreground">{angle}</p>
            </div>
          )}
          {proofPoint && (
            <div className="rounded border border-border bg-card px-2.5 py-1.5">
              ...
              <p className="text-foreground">{proofPoint}</p>
            </div>
          )}
        </div>
      )}

      {reasoning && <p className="mt-2 text-xs italic text-foreground/85">{reasoning}</p>}

      {steps.length > 0 && (
        <details className="mt-2 group" open>
          <summary className="cursor-pointer text-xs font-semibold text-foreground hover:text-brand-600">
            ✉ Sequence ({steps.length} steps)
          </summary>
          <ol className="mt-2 space-y-2">
            {steps.map((step, i) => (
              <SequenceStepRow key={i} step={step} />
            ))}
          </ol>
        </details>
      )}
      ...
      {notes.length > 0 && (
        <details className="mt-2 group">
          <summary className="cursor-pointer text-xs font-medium text-foreground/85 hover:text-brand-600">
            Personalization notes ({notes.length})
          </summary>
          <ul className="mt-1.5 ml-4 list-disc space-y-1 text-xs text-foreground/85">
            {notes.map((n, i) => (
              <li key={i}>{n}</li>
            ))}
          </ul>
      ...
      {flags.length > 0 && (
        <div className="mt-2 rounded border border-amber-200 bg-amber-50 px-2 py-1 text-[11px] text-amber-800 dark:bg-amber-950/40 dark:text-amber-300">
...
function SequenceStepRow({ step }: { step: Record<string, unknown> }) {
  const stepNumber = numberOrUndef(step.stepNumber) ?? 0;
  const channel = stringOr(step.channel, 'email');
  const dayOffset = numberOrUndef(step.dayOffset) ?? 0;
  const subject = stringOr(step.subject, '');
  const variant = stringOr(step.subjectVariant, '');
  const body = stringOr(step.body, '');
  const cta = stringOr(step.callToAction, '');
  const notes = stringOr(step.notes, '');
  ...
  return (
    <li className="rounded border border-border bg-muted/30 p-2.5">
      <div className="mb-1.5 flex items-center gap-2 text-[11px] font-medium uppercase tracking-wide text-muted-foreground">
        <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-brand-gradient text-[10px] font-semibold text-white">
          {stepNumber}
        </span>
        <span>{channelEmoji} {channel}</span>
        <span>·</span>
        <span>
          {dayOffset === 0 ? 'send now' : `+${dayOffset}d`}
        </span>
      </div>
      ...
      {variant && (
        <p className="text-xs text-foreground/85">
          <span className="text-muted-foreground">A/B variant:</span> {variant}
        </p>
      )}

      {body && (
        <pre className="mt-1.5 whitespace-pre-wrap font-sans text-xs leading-relaxed text-foreground/85">
          {body}
        </pre>
      )}
      ...
      {notes && (
        <p className="mt-1 text-[11px] italic text-muted-foreground">📝 {notes}</p>
      )}
    </li>
  );
}
